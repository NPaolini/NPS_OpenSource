...
P_Expr::errCode operVariable::calc(P_Expression* owner, P_Expr::exprVar& result, opNode* sender)
{
  ...
  int offset;
  int step;
  sender->Val.get_variable(id, offset, step);
//  offset += step * owner->getCycle();
  bool exist = true;
  result = owner->getVariable(id, offset, step * owner->getCycle(), exist);
  if(!exist) {
    setIdVarG(id);
...
//  offset += step * owner->getCycle();
  bool exist = true;
  result = F->getVariable(id, offset, step * owner->getCycle(), exist);
  if(!exist) {
    setIdVarGFunct(id);
    return P_Expr::UNDEFINED_VARIABLE;
    }
...
P_Expr::errCode operBlock::calc(P_Expression* owner, P_Expr::exprVar& result, opNode* sender)
{
  ...
  int offset;
  int step;
  sender->left->Val.get_variable(id, offset, step);
...
P_Expr::errCode operBlock2::calc(P_Expression* owner, P_Expr::exprVar& result, opNode* sender)
{
  ...
  int offset;
  int step;
  sender->left->Val.get_variable(id1, offset, step);
  bool exist = true;
  result = owner->getVariable(id1, 0, step * owner->getCycle(), exist);
  if(!exist)
    return P_Expr::UNDEFINED_VARIABLE;
  ...
  int id2;
  more->Val.get_variable(id2, offset, step);
  v = owner->getVariable(id2, 0, step * owner->getCycle(), exist);
  if(!exist)
    return P_Expr::UNDEFINED_VARIABLE;
  ...
  for(int i = 1; i < count; ++i) {
    P_Expr::exprVar v1 = owner->getVariable(id1, 0, i * blkStep1, exist);
    P_Expr::exprVar v2 = owner->getVariable(id2, 0, i * blkStep2, exist);
    err = calcWithCheckNullVal(result, v1, v2);
    if(err > P_Expr::NO_ERR || onShortCircuit(result))
      break;
    }
