...
svmObjLine::svmObjLine(PWin* par, int id, int x1, int y1, int x2, int y2) :
  svmObject(par, id, oLINE, x1, y1, x2, y2)
{
  init();
}
//-----------------------------------------------------------
svmObjLine::svmObjLine(PWin* par, int id, const PRect& r) :
  svmObject(par, id, oLINE, r)
{
  init();
}
//-----------------------------------------------------------
...
//-----------------------------------------------------------
void svmObjLine::setCommonProperty(const unionProperty& prop)
{
...
//-----------------------------------------------------------
void svmObjLine::setRect(const PRect& rect)
{
...
void svmObjLine::makeEquation()
{
  ...
  POINT pt2 = { r.right, r.bottom };
  if(lineProperty::edTopRight == rp->dir) {
    pt1.x = r.right;
    pt2.x = r.left;
    }
  lEq.setPoints(pt1, pt2);
#else
  lineProperty* rp = dynamic_cast<lineProperty*>(Prop);
  const PRect& r = Prop->Rect;
  if(r.left == r.right)
...
#if 1
  int x = lEq.getX(frame.top);
  if(isInX(x, r))
    return true;
  x = lEq.getX(frame.bottom);
  if(isInX(x, r))
    return true;

  int y = -lEq.getY(frame.left);
  if(isInY(y, r))
    return true;
  y = -lEq.getY(frame.right);
  if(isInY(y, r))
    return true;
#else
...
//-----------------------------------------------------------
void svmObjLine::copyPropFrom(const Property* other)
{
  *Prop = *other;
  makeEquation();
}
...
//-----------------------------------------------------------
void svmObjLine::restoreByHistoryObject(const svmObjHistory& history)
{
...
bool svmObjLine::load(LPCTSTR row)
{
  ...
  _stscanf_s(row, _T("%d,%d,%d,%d"),  &x1, &y1, &x2, &y2);
  if(x1 > x2) {
    if(y1 > y2) {
      int t = x1;
      x1 = x2;
