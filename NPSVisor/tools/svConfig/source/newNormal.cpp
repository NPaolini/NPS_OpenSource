...
//----------------------------------------------------------------------------
void addStringToComboBox(HWND hList, LPCTSTR str, int addSize)
{
...
//  int add = 0;
  for(int i = 0; i < SIZE_A(prf); ++i)
    addStringToComboBox(hwnd, prf[i], add);
  if(select < 0 || select >= SIZE_A(prf))
    select = 0;
...
void fillCBType(HWND hwnd, int select)
{
  ...
  int add = 0;
  for(int i = 0; i < SIZE_A(type); ++i)
    addStringToComboBox(hwnd, type[i], add);
  if(select < 0 || select >= SIZE_A(type))
    select = 0;
  SendMessage(hwnd, CB_SETCURSEL, select, 0);
}
//----------------------------------------------------------------------------
...
{
  TCHAR buff[50];
  _stprintf(buff, _T("%.5f"), val);
  SetWindowText(*Val, buff);
  _stprintf(buff, _T("%.5f"), vMin);
  SetWindowText(*Min, buff);
  _stprintf(buff, _T("%.5f"), vMax);
...
class newNormal
{
  public:
    ...
    static void load(LPCTSTR normFile);
  private:
...
//----------------------------------------------------------------------------
void newNormal::load(LPCTSTR normFile)
{
...
*/
//----------------------------------------------------------------------------
bool setupNormal(LPCTSTR normFile, PWin* parent)
{
  if(IDOK == PD_Normalz(parent, normFile).modal()) {
    newNormal::load(normFile);
    return true;
    }
  return false;
}
//----------------------------------------------------------------------------
void initNormal(LPCTSTR normFile)
{
  newNormal::load(normFile);
}
