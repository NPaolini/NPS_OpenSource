...
struct paramCom
{
  ...
  struct _b {
    ...
    public:
      _b(ePort p, eBaudRate b, eBitStop s, eParity pr, eBitLen l, eDtrDsr dtr,
  ...
  paramCom(ePort p = Com2, eBaudRate b = _9600, eBitStop s = ONE,
           eParity pr = NO_PARITY, eBitLen l = _8, eDtrDsr dtr = ON_DTR_DSR,
           eRtsCts rts = ON_RTS_CTS, eXonXoff x = NO_XON_XOFF) :
           b(p, b, s, pr, l, dtr, rts, x) {}
  paramCom(_b v) : b(v) {}
  paramCom(DWORD v) : b(*reinterpret_cast<_b*>(&v)) {}
