...
//------------------------------------------------------------------------
  struct exprVar
  {
    ...
    void setVariable(int id, int offset, int step) {
      type = tvInt;
      LARGE_INTEGER li;
      li.LowPart = id;
    ...
    void getVariable(int& id, int& offset, int& step) {
      LARGE_INTEGER li;
      li.QuadPart = iValue;
      id = li.LowPart;
...
//------------------------------------------------------------------------
  class node
  {
    public:
      ...
      node* left;
      node* right;
      // more ? usato per funzioni con pi? di due argomenti, i primi due
...
class P_Expression
{
  public:
    ...
    // currCycle ? usato in abbinamento a variabili in cui ? stato impostato lo step
  ...
  private:
    ...
    pTree makeLevel0_0(); // livelli di priorit? 0 == pi? basso
    pTree makeLevel0_1();
    pTree makeLevel0_2();
    pTree makeLevel1_0();
    pTree makeLevel1_1();
