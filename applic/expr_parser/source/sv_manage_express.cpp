...
#include <shlwapi.h>
//------------------------------------------------------------------
svManExpression::svManExpression(pInfoCallBack p_info_fz) : currState(sUndef), pManVars(allocManVars(p_info_fz)), ReplaceVar(0),
  pManFunct(0), Error(false), pInfoFz(p_info_fz) {}
//------------------------------------------------------------------
...
//------------------------------------------------------------------
void svManExpression::resetOrigBuff()
{
  for(uint i = 0; i < SIZE_A(origBuff); ++i) {
    safeDeleteA(origBuff[i].buff);
    origBuff[i].dim = 0;
  }
}
//------------------------------------------------------------------
...
//------------------------------------------------------------------
bool svManExpression::makeExpressionByBuff(LPCTSTR buff)
{
  LPTSTR buff2 = str_newdup(buff);
  return makeExpressionByBuff(buff2);
}
//------------------------------------------------------------------
bool svManExpression::makeExpressionByBuff(LPTSTR buff)
{
  int nElem = pExpr.getElem();
  for(int i = 0; i < nElem; ++i)
    delete pExpr[i].Expr;
  ...
  if(!buff)
    return false;
  infoBuff partBuff[epbMax];
  for(uint i = 0; i < SIZE_A(partBuff); ++i) {
    partBuff[i].buff = findAndLoadblock(buff, Marker[i], partBuff[i].dim);
    if(partBuff[i].buff) {
      origBuff[i].buff = str_newdup(partBuff[i].buff);
      origBuff[i].dim = partBuff[i].dim;
...
bool svManExpression::makeExpression(LPCTSTR path)
{
  ...
  if(!buff)
    return false;
  return makeExpressionByBuff(buff);
}
//------------------------------------------------------------------
...
//------------------------------------------------------------------
P_Expr::exprVar svManExpression::getVariable(int id, int offset, bool& exist)
{
  return pManVars->getVariable(id, offset, exist);
}
//---------------------------------------------------------------------
//---------------------------------------------------------------------
//---------------------------------------------------------------------
...
//------------------------------------------------------------------
P_Expr::exprVar svCustFunct::getVariable(int id, int offset, bool& exist)
{
  exist = true;
  if(id >= MAX_PARAM_ID) {
    id -= MAX_PARAM_ID;
