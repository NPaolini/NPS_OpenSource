...
//----------------------------------------------------------------------------
bool progressBar::setCount(DWORD count, LPCTSTR info)
{
  if(Aborted)
    return false;
  currPos = count;
  ...
  TCHAR buff[1024];
  wsprintf(buff, Message, count, maxCount, info);
  SetDlgItemText(*this, IDC_STATICTEXT_PROGRESS, buff);
  return true;
}
//----------------------------------------------------------------------------
bool progressBar::addCount(DWORD count, LPCTSTR info)
{
  return setCount(currPos + count, info);
}
