 ...
 */
lstring
ntlm_fetch_string(lstring& rv, const ntlmhdr *packet, const int32_t packet_size, const strhdr * str, const uint32_t flags)
{
    static char buf[NTLM_MAX_FIELD_LENGTH + 2];
//    lstring rv;
    char *d;
...
#define CP_ CP_ACP
//#define CP_ CP_UTF8
//#define WC_FLAGS WC_COMPOSITECHECK | WC_DISCARDNS | WC_SEPCHARS | WC_DEFAULTCHAR | WC_NO_BEST_FIT_CHARS
#define WC_FLAGS WC_COMPOSITECHECK | WC_SEPCHARS
//#define WC_FLAGS 0
      BOOL useDef = true;
      int len = WideCharToMultiByte(CP_, WC_FLAGS, (LPCWSTR)rv.str, l >> 1, buf, sizeof(buf) - 1, NULL, &useDef);
//      int len = WideCharToMultiByte(CP_, WC_FLAGS, (LPCWSTR)rv.str, l >> 1, buf, sizeof(buf), NULL, NULL);
      replaceAcc(buf);
      rv.l = len;
      rv.str = buf;
#else
      LPWSTR s = (LPWSTR)rv.str;
      rv.str = d = buf;
...
/**/
    rv = ntlm_fetch_string(rv, &auth->hdr, size, &auth->domain, auth->flags);
    if (rv.l >= DNLEN) {
    ...
    rv = ntlm_fetch_string(rv, &auth->hdr, size, &auth->user, auth->flags);
    if (rv.l >= UNLEN) {
        debug("ntlm_unpack_auth: User length %d too big for %d byte packet.\n", rv.l , UNLEN);
        return NTLM_ERR_BLOB;
    }
