...
#define NTV_DEFAULT_DOMAIN _T(".")

extern LPTSTR NTAllowedGroup;
extern LPTSTR NTDisAllowedGroup;
extern int UseDisallowedGroup;
extern int UseAllowedGroup;
extern int debug_enabled;
extern TCHAR Default_NTDomain[DNLEN+1];
extern LPCTSTR errormsg;
 ...
 */
int Valid_User(LPSTR UserName, LPSTR Password, LPSTR Group);
...
#define debug(X...) if (debug_enabled) { \
                    fprintf(stderr,_T("nt_auth[%d](%s:%d): "), getpid(), \
...
debug(LPTSTR format,...)
{
    if (debug_enabled) {
		...
		_ftprintf(stderr, _T("nt_auth[%d]: "),getpid());
