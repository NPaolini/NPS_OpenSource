...

{
    ...
    NTSTATUS status;
    PPOLICY_PRIMARY_DOMAIN_INFO ppdiDomainInfo = 0;
...
//----------------------------------
static void separateUser(LPCSTR cred, LPSTR user, LPSTR dom)
{
  LPCSTR p = cred;
  dom[0] = 0;
  user[0] = 0;
 ...
 */
int
ntlm_check_auth(ntlm_authenticate * auth, char *user, char *domain, int auth_length, const char** token)
{
    int x;
    int rv;
    ...
    rv = SSP_ValidateNTLMCredentials(auth, auth_length, credentials, token);
    ...
    debug("credentials: %s\n", credentials);
#if 0
    separateUser(credentials, user, domain);
#endif
...
process_options(int argc, char *argv[])
{
    ...
    while (-1 != (opt = getopt(argc, argv, "hdvA:D:"))) {
        ...
        case 'A':
            ...
            NTAllowedGroup= _tcsdup(optarg);
            UseAllowedGroup = 1;
            break;
        case 'D':
          delete[]NTDisAllowedGroup;
            NTDisAllowedGroup= _tcsdup(optarg);
...
token_decode(size_t *decodedLen, uint8_t decoded[], const char *buf)
{
    ...
    return true;
#else
  *decodedLen = base64_decode((LPSTR)decoded, buf, strlen(buf));
//  const char *dec = base64_decode(buf, (int*)decodedLen);
//  if(!dec || !*decodedLen) {
  if(!*decodedLen) {
    SEND_BH("message=\"base64 decode failed\"");
    fprintf(stderr, "ERROR: base64 decoding failed for: '%s'\n", buf);
...
manage_request()
{
    ...
    static char domain[DNLEN + UNLEN + 2];
    ...
    if (memcmp(buf, "KK ", 3) == 0) {   /* authenticate-request */
      ...
      switch (fast_header->type) {
        ...
        case NTLM_AUTHENTICATE: {
            /* check against SSPI */
            const char* token = 0;
            int err = ntlm_check_auth((ntlm_authenticate *) decoded, user, domain, decodedLen, &token);
...
//            fprintf(stdout, "OK user=\"%s\\%s\"\n", domain, user);
    #if 1
            if(true || token)
              fprintf(stdout, "OK user=\"%s\\%s\" token=%s\n", domain, user, token);
            else
              fprintf(stdout, "OK user=\"%s\\%s\"\n", domain, user);
    #else
            if(user[0])
              ...
              fprintf(stdout, "OK user=\"%s\" token=%s\n", domain, token);
    #endif
#endif
#endif
            return 1;
...
#define VERSION "4.5-20190128-r568e66b7c"
