...
BOOL GenServerContext(PAUTH_SEQ, PVOID, DWORD, PVOID, PDWORD, PBOOL, LPTSTR);

static HMODULE hModule;
static int NTLM_mode = SSP_BASIC;
static LPTSTR SSP_Package_InUse;
...
HMODULE LoadSecurityDll(int mode, LPCTSTR SSP_Package)
{
...
//    lstrcpy(lpszDLL, _T(WIN2K_SECURITY_DLL));
    ...
    /* Get max token size */
  _QuerySecurityPackageInfo((SECURITY_PSTR)SSP_Package, &pSPI);
//  _QuerySecurityPackageInfo((SEC_CHAR*)__T(SSP_Package), &pSPI);
  cbMaxToken = pSPI->cbMaxToken;
    _FreeContextBuffer(pSPI);

    /* Allocate buffers for client and server messages */
#if true
    pClientBuf = new uint8_t[cbMaxToken + 4];
    pServerBuf = new uint8_t[cbMaxToken + 4];
    encoded = new char[(cbMaxToken / 3 + 1) * 4 + 4];
    SSP_Package_InUse = _tcsdup(SSP_Package);
#else
                      ...
                      PVOID pIn, DWORD cbIn, PVOID pOut, PDWORD pcbOut, PBOOL pfDone)
{
    ...
    if (!pAS->fInitialized) {
        SecurityStatus = _AcquireCredentialsHandle(NULL, (SECURITY_PSTR) SSP_Package_InUse,
...
BOOL GenServerContext(PAUTH_SEQ pAS, PVOID pIn, DWORD cbIn, PVOID pOut,
                      PDWORD pcbOut, PBOOL pfDone, LPTSTR credentials)
{
    /*
     *   Routine Description:
     *
    ...
    SecBuffer       sbIn;
    ULONG           fContextAttr;
    SecPkgContext_Names namebuffer = { 0 };
    ...
    if ((credentials != NULL) &&
...
#if SSP_DEBUG
        fprintf(stderr, "QueryContextAttributes returned: %x. Credential:%s\n", SecurityStatus, namebuffer.sUserName);
#endif
        if (SecurityStatus < 0) {
#if SSP_DEBUG
        ...
        _tcsncpy(credentials, namebuffer.sUserName, SSP_MAX_CRED_LEN);

    }
    if(namebuffer.sUserName)
      _FreeContextBuffer(namebuffer.sUserName);
    ...
    *pfDone = !(SecurityStatus == SEC_I_CONTINUE_NEEDED
                || SecurityStatus == SEC_I_COMPLETE_AND_CONTINUE);
#if SSP_DEBUG
        fprintf(stderr, "cbOut=%d, done=%d\n", *pcbOut, *pfDone);
#endif
    return TRUE;
}

#ifdef _UNICODE
#define U_PARA (unsigned short*)
#else
#define U_PARA (unsigned char*)
#endif
BOOL WINAPI SSP_LogonUser(PTSTR szUser, PTSTR szPassword, PTSTR szDomain)
{
    ...
    do {
        ...
        ai.Domain = U_PARA szDomain;
        ai.DomainLength = lstrlen(szDomain);
        ai.User = U_PARA szUser;
        ai.UserLength = lstrlen(szUser);
        ai.Password = U_PARA szPassword;
...
const char * WINAPI SSP_MakeChallenge(PVOID PNegotiateBuf, int NegotiateLen)
{
    ...
    if (fResult != NULL) {
        ...
        base64_encode(encoded, (char *) fResult, cbOut);
        return encoded;
    }
    return NULL;
}
#if 0
const char * WINAPI SSP_ValidateNTLMCredentials(PVOID PAutenticateBuf, int AutenticateLen, LPTSTR credentials)
{
    BOOL        fDone      = FALSE;
    BOOL        fResult    = FALSE;
    DWORD       cbOut      = 0;
    ...
    if (cbOut > 0 && fResult) {
        base64_encode(encoded, (char *) pServerBuf, cbOut);
        return encoded;
...
#else
BOOL WINAPI SSP_ValidateNTLMCredentials(PVOID PAutenticateBuf, int AutenticateLen, LPTSTR credentials, const char** token)
{
    BOOL        fDone      = FALSE;
    BOOL        fResult    = FALSE;
    DWORD       cbOut      = 0;
...
#endif
const char * WINAPI SSP_MakeNegotiateBlob(PVOID PNegotiateBuf, int NegotiateLen, PBOOL fDone, int * Status, LPTSTR credentials)
{
    DWORD       cbOut      = 0;
    DWORD       cbIn       = 0;
...
const char * WINAPI SSP_ValidateNegotiateCredentials(PVOID PAutenticateBuf, int AutenticateLen, PBOOL fDone, int * Status, LPTSTR credentials)
{
    DWORD       cbOut      = 0;
    DWORD       cbIn       = 0;
