...
#define NEGOTIATE_PACKAGE_NAME _T("Negotiate")
...
HMODULE LoadSecurityDll(int, LPCTSTR);
void UnloadSecurityDll(void);
BOOL WINAPI SSP_LogonUser(PTSTR, PTSTR, PTSTR);
//const char * WINAPI SSP_ValidateNTLMCredentials(PVOID, int, char *);
BOOL WINAPI SSP_ValidateNTLMCredentials(PVOID, int, char *, const char** token);
const char * WINAPI SSP_ValidateNegotiateCredentials(PVOID, int, PBOOL, int *, LPTSTR);
const char * WINAPI SSP_MakeChallenge(PVOID, int);
const char * WINAPI SSP_MakeNegotiateBlob(PVOID, int, PBOOL, int *, LPTSTR);
